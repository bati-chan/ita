...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!message.guild.me.hasPermission("MANAGE_MESSAGES")) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You do not have permission to use this command!**`)).then(m => m.delete(5000));
    if (!toUnmute) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must mention a user to unmute!**`)).then(m => m.delete(5000));
    if (toUnmute.id === message.member.id) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You cannot unmute yourself!**`)).then(m => m.delete(5000));
    
    try {
      if (!toUnmute.roles.has(muteRole.id)) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| This user is not muted!**`)).then(m => m.delete(5000));
      ...
      return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must set up a mute role first and have someone muted!**`).setFooter(`To set up a mute role for muting users, use ${prefix}${setMute}`)).then(m => m.delete(5000));
    }
  }
};
