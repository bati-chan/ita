...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (yourBal < amount) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must specify a smaller amount to give ${emote}${name}(s)!**`)).then(m => m.delete(5000));
    yourBal = await db.subtract(`balance_${message.member.id}`, amount);
    memberBal = await db.add(`balance_${member.user.id}`, amount);
