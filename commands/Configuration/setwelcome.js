...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!selection) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must specify a selection to configure your welcome message!**`)).then(m => m.delete(5000));
    if (!welcomeChannel) return message.channel.send(cancelEmbed.setColor("#F51B00").setDescription(`❌ **| You must configure a welcome channel first!**`)).then(m => m.delete(5000));
    ...
    if (selection.toLowerCase() === "top message") {
      ...
      message.channel.send(topEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(top => {
        if (top.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
        if (top.first().content.includes("{welcome}")) if (!welcomeRole) return message.channel.send(cancelEmbed.setColor("#F51B00").setDescription(`❌ **| You must configure a welcome role first!**`)).then(m => m.delete(5000));
      ...
      message.channel.send(authorPicEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(authorPic => {
        if (authorPic.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(authorTextEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(authText => {
        if (authText.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(titleEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(title => {
        if (title.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(descEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(desc => {
        if (desc.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(thumbnailEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(thumbnail => {
        if (thumbnail.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(imageEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(image => {
        if (image.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(colorEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(color => {
        if (color.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **Successfully cancelled welcome configuration!**`));
      ...
      const footerEmbed = new RichEmbed()
        ...
        .setThumbnail(client.user.displayAvatarURL)
        .setDescription(`What would you like your ${selection.toLowerCase()} to be?`)
        .addField(`Important Syntax`, `Type \`{server size}\` to display the count of members after the new member joins.`)
      ...
      message.channel.send(footerEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(footer => {
        if (footer.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
      ...
      message.channel.send(timeEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(time => {
        if (time.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
        ...
        else return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must specify a valid choice to set up a timestamp in your welcome message!**`)).then(m => m.delete(5000));
        return message.channel.send(successEmbed);
      })).catch(e => console.log(e));
    } else if (selection.toLowerCase === "logs") {
      const logsEmbed = new RichEmbed()
        .setColor("#38013A")
        .setAuthor(`Custom Welcome Message Configuration`)
        .setThumbnail(client.user.displayAvatarURL)
        .setDescription(`Would you like ${selection.toLowerCase()} to be visible (easier moderation)?`)
        .addField(`Important Syntax`, `You must type either \`yes\` to enable logs or \`no\` to disable logs.`)
        .setFooter(`You may type 'cancel' at any time to cancel configuring your welcome message.`);
      message.channel.send(logsEmbed).then(() => message.channel.awaitMessages(filter, { max: 1, time: 1000 * 60 * 5 }).then(logs => {
        if (logs.first().content.toLowerCase() === "cancel") return message.channel.send(cancelEmbed.setColor("#38013A").setDescription(`✅ **| Successfully cancelled welcome configuration!**`));
        else if (logs.first().content.toLowerCase() === "yes") db.set(`welcomeLogs_{message.guild.id}`, true);
        else if (logs.first().content.toLowerCase() === "no") db.set(`welcomeLogs_{message.guild.id}`, false);
        else return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must specify a valid choice to configure logs in your welcome message!**`)).then(m => m.delete(5000));
        return message.channel.send(successEmbed);
      })).catch(e => console.log(e));
    } else return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must specify a valid selection to configure your welcome message!**`)).then(m => m.delete(5000));
  }
}