...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      const aloneEmbed = new RichEmbed().setColor("#38013A").setImage("https://media.giphy.com/media/Y4z9olnoVl5QI/giphy.gif");
      return message.channel.send(`Aww, it's okay ${message.author}! I'll be here to hug you!~`, aloneEmbed);
    }
    if (!toHug) return message.channel.send(errorEmbed.setColor("#F51B00").setDescription(`❌ **| You must mention a user to hug!**`)).then(m => m.delete(5000));
